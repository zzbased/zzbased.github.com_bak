...
- NTU他们对预测的CTR进行了一个calibration，使得其与true clicked likelihood/average estimated CTR更接近，如果最后evaluation metric是logloss，这有时会有较大改进。FastML上面介绍这个 [Platt's Scaling (SVM输出概率的方法) and Isotonic Regression](http://fastml.com/classifier-calibration-with-platts-scaling-and-isotonic-regression/)。@范涛_中科大
